# EWC Backend — Architecture & API Contract
...
service. The service has been implemented (KAM-9); see the root
[`README.md`](../README.md) for setup, run, test, and container
build instructions.
...
|---|---|
| [ADR-001-target-architecture.md](./ADR-001-target-architecture.md) | Architecture Decision Record covering backend framework, service layering, repository layout, deployment topology, CLI↔backend communication, and AI agent-friendly API design. **Status: Implemented** (KAM-9). |
...
| [openapi.yaml](./openapi.yaml) | OpenAPI 3.1 specification covering all current CLI capabilities. The live spec is also auto-generated by FastAPI at `/openapi.json` when the server is running. |
